<template>
  <div class="list-container">
    ...
    <div class="search-bar">
      <input v-model="searchKeyword" placeholder="搜索发票..." class="search-input" />
    </div>
    ...
    <div v-if="showAddModal || editingItem" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        ...
        <form @submit.prevent="saveItem">
          ...
          <div class="form-group">
            <label>发票号</label>
            <input v-model="formData.invoice_no" required />
          </div>
          <div class="form-group">
            <label>发票金额</label>
...
const formData = ref({ contract_id: '', invoice_no: '', amount: '', date: '' })

const filteredInvoices = computed(() => {
  if (!searchKeyword.value) return invoices.value
  const kw = searchKeyword.value.toLowerCase()
  return invoices.value.filter(i =>
    (i.invoice_no && i.invoice_no.toLowerCase().includes(kw)) ||
    (i.contract_name && i.contract_name.toLowerCase().includes(kw))
  )
})
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await window.electronAPI.updateInvoice({ ...formData.value, id: editingItem.value.id })
    } else {
      await window.electronAPI.createInvoice(formData.value)
    }
    closeModal()
    await loadInvoices()
  } catch (err) {
...
const closeModal = () => {
  ...
  formData.value = { contract_id: '', invoice_no: '', amount: '', date: '' }
}
